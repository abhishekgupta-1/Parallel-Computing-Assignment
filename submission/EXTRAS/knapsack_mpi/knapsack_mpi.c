...
int main(int argc, char *argv[]){
    ...
    if (myrank == 0){ //master code
        ...
        for (i=1;i<nProc;i++){
            MPI_Isend(pair, 2, MPI_INT, i, 1, MPI_COMM_WORLD,&request);
            MPI_Isend(inp, n, MPI_PAIR, i, 2, MPI_COMM_WORLD,&request);
        }
        ...
        MPI_Isend(tempSol, n+1, MPI_INT, dst, PBM_TAG, MPI_COMM_WORLD,&request);
        while (idle != nProc-1){
            MPI_Status status;
            ...
            if (tag == BnB_TAG) {
                ...
                if (high > bestSolVal) {
                    ...
                    DBG("\n");
                    MPI_Isend(data, total+1, MPI_INT, src, BnB_TAG, MPI_COMM_WORLD,&request);
                    
                }
                else{
                    tag = DONE;
                    MPI_Isend(&tag, 1, MPI_INT, src, DONE, MPI_COMM_WORLD,&request);
                }
            }
        }
        for (i=1;i<nProc;i++){
            tag = END_TAG;
            MPI_Isend(&tag, 1, MPI_INT, i, END_TAG, MPI_COMM_WORLD,&request);
        }
        ...
        while (1){
            ...
            if (tag == PBM_TAG) {
                ...
                while (!empty_list(&list)){
                    ...
                    if (high >= bestSolVal){
                        ...
                        if (low >= bestSolVal)  {
                            auxSp[n] = low;
                            bestSolVal = low;
                            MPI_Isend(auxSp, n+1, MPI_INT, 0, SOLVE_TAG, MPI_COMM_WORLD,&request); //problem
                        }
                        if (low != high){ //problem is here
                            int data[2];
                            data[0] = high;
                            data[1] = list.len;
                            MPI_Isend(data, 2, MPI_INT, 0, BnB_TAG, MPI_COMM_WORLD,&request);
                            ...
                            if (status.MPI_TAG == BnB_TAG){
                                ...
                                for (i=0;i<total;i++){
                                    ...
                                    sp[n] = bestSolVal;
                                    MPI_Isend(sp, n+1, MPI_INT, assigned[i], PBM_TAG, MPI_COMM_WORLD,&request);
                ...
                tag = IDLE_TAG;
                MPI_Isend(&tag, 1, MPI_INT, 0, IDLE_TAG, MPI_COMM_WORLD,&request);
            }
        }
    }
    return 0;
