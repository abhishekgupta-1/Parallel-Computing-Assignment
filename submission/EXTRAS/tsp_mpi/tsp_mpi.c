...
int main(int argc, char *argv[]){
    ...
    if (myrank == 0){ //master code
        ...
        MPI_Isend(pb,n+4,MPI_INT,dst,PBM_TAG,MPI_COMM_WORLD,&request);
        while (idle != nProc-1){
            MPI_Status status;
            ...
            if (tag == BnB_TAG) {
                ...
                if (low < bestCost) {
                    ...
                    tag = DONE;
                    MPI_Isend(&bestCost, 1, MPI_INT, src, DONE, MPI_COMM_WORLD,&request);
                }
            }
        }
        for (i=1;i<nProc;i++){
                ...
                while (!empty_list(&list)){
                    ...
                    if (auxSp[n+3] == 1){
                        ...
                        if (auxSp[n+1] < bestCost){
                            bestCost = auxSp[n+1];
                            MPI_Isend(auxSp, n+4, MPI_INT, 0, SOLVE_TAG, MPI_COMM_WORLD,&request);
                        }
                        DBG("Continuing\n");
                        continue;
                    }
                    ...
                    if (low < bestCost){
                        ...
                        //if (low > bestSolVal)  {
                            ...
                            ////Isend bestSolVal and solution to the master
                        //}
                        int data[2];
                        data[0] = low;
                        data[1] = list.len;
                        ...
                        if (status.MPI_TAG == BnB_TAG){
                            ...
                            for (i=0;i<total;i++){
                                ...
                                MPI_Isend(sp, n+4, MPI_INT, assigned[i], PBM_TAG, MPI_COMM_WORLD,&request);
                            }
                        }
                        else{
